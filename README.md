...
Calvin S itself covers `a-z`, `A-Z`, `[ ] ! @ # $ % ^ & * - _ , . ?` and space.
All of it is here, `[` and `]` included.

Digits and the remaining punctuation are an **extension**. The reference font
defines no glyphs for them, so
[TAAG](https://patorjk.com/software/taag/#p=display&f=Calvin%20S) renders
...
most resemble: `0` is barred so it does not read as `o`, `8` closes its lower
...
Also added: `( ) : ; ' " / \`. Anything else is skipped, as it is upstream.
